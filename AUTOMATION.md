# 🤖 Автоматизация и мониторинг mydoc-loyalty

## 📋 Обзор

Система настроена с полной автоматизацией для обеспечения стабильной работы, автоматического восстановления и мониторинга.

---

## 🔄 Автоматические задачи (Cron)

### 1. **Проверка здоровья системы** (каждые 5 минут)
- **Скрипт:** `/tmp/mydoc-loyalty/health-check.sh`
- **Лог:** `/var/log/mydoc-health-check.log`
- **Расписание:** `*/5 * * * *`

**Что проверяет:**
- ✅ Работу всех Docker контейнеров
- ✅ Доступность API (https://it-mydoc.ru/api/health)
- ✅ Дисковое пространство (предупреждение при >85%)
- ✅ Срок действия SSL сертификата
- ✅ Автоматический перезапуск упавших контейнеров

**Действия при проблемах:**
- Автоматически перезапускает упавшие контейнеры
- Очищает дисковое пространство при необходимости
- Обновляет SSL сертификат если истекает

---

### 2. **Сбор метрик** (каждые 15 минут)
- **Скрипт:** `/tmp/mydoc-loyalty/system-monitor.sh`
- **Лог:** `/var/log/mydoc-metrics.log`
- **Расписание:** `*/15 * * * *`

**Собирает метрики:**
- 📊 CPU и RAM использование
- 💾 Дисковое пространство
- 🐳 Статус Docker контейнеров
- 📈 Статистика по каждому контейнеру (CPU/Memory)
- 💽 Размер базы данных
- 👥 Количество пользователей
- ⚠️ Количество ошибок в логах

---

### 3. **Резервное копирование** (ежедневно в 02:00)
- **Скрипт:** `/tmp/mydoc-loyalty/backup.sh`
- **Директория:** `/var/backups/mydoc-loyalty/`
- **Лог:** `/var/log/mydoc-backup.log`
- **Расписание:** `0 2 * * *`

**Создает бэкапы:**
- 📦 База данных PostgreSQL (сжатый SQL дамп)
- 📁 Загруженные файлы из `/uploads`
- 🗑️ Автоматически удаляет бэкапы старше 7 дней

**Формат имен:**
```
db_backup_YYYYMMDD_HHMMSS.sql.gz
uploads_backup_YYYYMMDD_HHMMSS.tar.gz
```

---

### 4. **Обновление SSL сертификата** (каждые 12 часов)
- **Расписание:** `0 */12 * * *`
- **Лог:** `/var/log/certbot-renew.log`

**Действия:**
- 🔐 Проверяет срок действия сертификата
- ✅ Обновляет при необходимости
- 🔄 Автоматически перезапускает nginx после обновления

---

### 5. **Очистка Docker** (еженедельно, воскресенье в 03:00)
- **Расписание:** `0 3 * * 0`
- **Лог:** `/var/log/docker-cleanup.log`

**Удаляет:**
- 🗑️ Неиспользуемые Docker образы (старше 7 дней)
- 🗑️ Неиспользуемые Docker контейнеры
- 🗑️ Неиспользуемые Docker volumes
- 🗑️ Build cache

---

### 6. **Еженедельный перезапуск** (понедельник в 04:00)
- **Расписание:** `0 4 * * 1`
- **Лог:** `/var/log/mydoc-restart.log`

**Причины:**
- 🔄 Освобождение памяти
- 🔄 Сброс кэшей
- 🔄 Применение обновлений

---

## 🐳 Docker Health Checks

Все контейнеры имеют встроенные health checks:

### PostgreSQL
```yaml
healthcheck:
  test: ["CMD-SHELL", "pg_isready -U mydoc_user -d mydoc_loyalty"]
  interval: 30s
  timeout: 10s
  retries: 3
  start_period: 40s
```

### Redis
```yaml
healthcheck:
  test: ["CMD", "redis-cli", "ping"]
  interval: 30s
  timeout: 10s
  retries: 3
  start_period: 10s
```

### Backend
```yaml
healthcheck:
  test: ["CMD", "curl", "-f", "http://localhost:8000/health"]
  interval: 30s
  timeout: 10s
  retries: 3
  start_period: 60s
```

**Политика перезапуска:** `restart: unless-stopped`
- Контейнеры автоматически перезапускаются при падении
- Не перезапускаются если остановлены вручную

---

## 📊 Просмотр логов

### Логи проверки здоровья
```bash
tail -f /var/log/mydoc-health-check.log
```

### Логи метрик
```bash
tail -f /var/log/mydoc-metrics.log
```

### Логи резервного копирования
```bash
tail -f /var/log/mydoc-backup.log
```

### Логи обновления SSL
```bash
tail -f /var/log/certbot-renew.log
```

### Логи Docker контейнеров
```bash
cd /tmp/mydoc-loyalty
docker-compose logs -f backend
docker-compose logs -f --tail=100 backend  # Последние 100 строк
```

---

## 🔧 Ручное управление

### Запуск проверки здоровья вручную
```bash
/tmp/mydoc-loyalty/health-check.sh
```

### Создание бэкапа вручную
```bash
/tmp/mydoc-loyalty/backup.sh
```

### Сбор метрик вручную
```bash
/tmp/mydoc-loyalty/system-monitor.sh
```

### Восстановление из бэкапа
```bash
# Восстановление базы данных
gunzip < /var/backups/mydoc-loyalty/db_backup_YYYYMMDD_HHMMSS.sql.gz | \
  docker exec -i mydoc_postgres psql -U mydoc_user mydoc_loyalty

# Восстановление файлов
tar -xzf /var/backups/mydoc-loyalty/uploads_backup_YYYYMMDD_HHMMSS.tar.gz -C /tmp/mydoc-loyalty/
```

### Проверка статуса cron задач
```bash
cat /etc/cron.d/mydoc-loyalty
systemctl status cron
```

### Перезапуск контейнеров вручную
```bash
cd /tmp/mydoc-loyalty
docker-compose restart
docker-compose restart backend  # Только backend
```

---

## 📈 Мониторинг метрик

### Просмотр последних метрик
```bash
tail -20 /var/log/mydoc-metrics.log
```

### Поиск проблем
```bash
# Поиск ошибок
grep "ERROR" /var/log/mydoc-health-check.log

# Поиск критических проблем
grep "КРИТИЧНО" /var/log/mydoc-health-check.log

# Проверка использования CPU
grep "CPU:" /var/log/mydoc-metrics.log | tail -20

# Проверка использования RAM
grep "RAM:" /var/log/mydoc-metrics.log | tail -20
```

---

## 🔔 Настройка уведомлений (опционально)

Для получения уведомлений о проблемах можно добавить в скрипты отправку email или Telegram:

### Telegram уведомления
```bash
# В health-check.sh добавить:
TELEGRAM_BOT_TOKEN="your_bot_token"
TELEGRAM_CHAT_ID="your_chat_id"

send_telegram() {
    curl -s -X POST "https://api.telegram.org/bot$TELEGRAM_BOT_TOKEN/sendMessage" \
        -d chat_id="$TELEGRAM_CHAT_ID" \
        -d text="$1" > /dev/null
}

# При критической ошибке:
send_telegram "🚨 КРИТИЧНО: mydoc_backend не удалось перезапустить!"
```

---

## 📝 Ротация логов

Настроена автоматическая ротация логов в `/etc/logrotate.d/mydoc-loyalty`:

- **Ежедневная ротация** для логов health-check и metrics
- **Хранение** последних 30 дней
- **Автоматическое сжатие** старых логов

Проверка конфигурации:
```bash
logrotate -d /etc/logrotate.d/mydoc-loyalty
```

Ручная ротация:
```bash
logrotate -f /etc/logrotate.d/mydoc-loyalty
```

---

## 🛡️ Безопасность

### Права доступа к файлам
```bash
chmod 600 /tmp/mydoc-loyalty/.env
chmod 600 /tmp/mydoc-loyalty/backend/.env
chmod 700 /tmp/mydoc-loyalty/*.sh
chmod 700 /var/backups/mydoc-loyalty
```

### Регулярная проверка безопасности
```bash
# Проверка портов
netstat -tulpn | grep -E ":(80|443|3000|5432|6379|8000)"

# Проверка SSL сертификата
openssl x509 -in /etc/letsencrypt/live/it-mydoc.ru/fullchain.pem -noout -dates -subject

# Проверка обновлений Docker образов
docker-compose pull
```

---

## 📞 Поддержка

При проблемах проверьте:
1. ✅ Статус контейнеров: `docker-compose ps`
2. ✅ Логи health-check: `tail -f /var/log/mydoc-health-check.log`
3. ✅ API: `curl https://it-mydoc.ru/api/health`
4. ✅ Логи backend: `docker-compose logs backend | tail -50`

Все скрипты автоматизации находятся в `/tmp/mydoc-loyalty/`

